...
router.get('/', auth, async (req, res) => {
    try {
        const history = await AiHistory.find({ userId: req.user.id }).sort({ timestamp: -1 }).limit(20);
        res.json(history);
    } catch (err) {
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        if (!moduleType || !prompt || !response) {
            return res.status(400).json({ error: 'Module, prompt, and response are required.' });
        }
