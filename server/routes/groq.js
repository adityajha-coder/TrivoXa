...
router.post('/chat', auth, async (req, res) => {
    try {
        ...
        if (statusCode === 401) {
            return res.status(401).json({ error: 'Unauthorized: API key is invalid or expired', message: errorMessage });
        }
        if (statusCode === 429) {
            return res.status(429).json({ error: 'Rate limited: Too many requests', message: 'Wait a moment and try again' });
        }

        res.status(statusCode).json({ error: 'Groq API error', message: errorMessage });
    }
});
