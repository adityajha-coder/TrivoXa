const CodeGitExplorerPage = {
  ...
  _applyAuthGate() {
    ...
    if (API.getAuthToken()) {
      ...
      if (exploreBtn) {
        ...
        exploreBtn.style.opacity = "";
      }
      if (typeTabs) {
        typeTabs.style.pointerEvents = "";
        typeTabs.style.opacity = "";
      }
    ...
    if (typeTabs) {
      typeTabs.style.pointerEvents = "none";
      typeTabs.style.opacity = "0.4";
    }
    ...
    if (emptyState) {
      ...
      document
        .getElementById("explorer-gate-login")
  ...
  bindEvents() {
    ...
    document
      .getElementById("ai-summary-refresh")
      ?.addEventListener("click", () => {
        if (!API.requireAuth()) return;
        if (this.repoData?.repo)
          this.generateAiSummary(this.repoData.repo, this.repoData.tree || []);
      });
    document
      .getElementById("ai-summary-close")
      ?.addEventListener("click", () => {
    ...
    document.getElementById("explorer-tabs")?.addEventListener("click", (e) => {
      ...
      if (!tab) return;
      document
        .querySelectorAll("#explorer-tabs .tab-item")
        .forEach((t) => t.classList.remove("active"));
      tab.classList.add("active");
      this.activeView = tab.dataset.view;
      document.getElementById("explorer-structure-view").style.display =
        this.activeView === "structure" ? "block" : "none";
      document.getElementById("explorer-git-view").style.display =
        this.activeView === "git" ? "block" : "none";
    });

    document
      .getElementById("explorer-type-tabs")
      ?.addEventListener("click", (e) => {
        const tab = e.target.closest(".tab-item");
        if (!tab) return;
        if (!API.requireAuth()) return;
        document
          .querySelectorAll("#explorer-type-tabs .tab-item")
          .forEach((t) => t.classList.remove("active"));
        tab.classList.add("active");
        ...
        if (this.searchType === "local") {
          ...
          document.querySelector(".input-icon.fa-github").style.display =
            "none";
        } else {
          ...
          if (this.searchType === "user") {
            ...
            input.placeholder =
              "e.g. facebook/react or https://github.com/vuejs/core";
          }
        }
      });

    document
      .getElementById("local-folder-input")
  ...
  async load() {
    ...
    try {
      ...
      document.getElementById("explorer-repo-name").textContent =
        repo.full_name;
      document.getElementById("explorer-lang").textContent =
        repo.language || "";
      document.getElementById("explorer-stars").innerHTML =
        `<i class="fa-solid fa-star"></i> ${Helpers.formatNumber(repo.stargazers_count)}`;
      document.getElementById("explorer-forks").innerHTML =
        `<i class="fa-solid fa-code-fork"></i> ${Helpers.formatNumber(repo.forks_count)}`;
...
Keep it concise (under 200 words). Do not use code blocks.`;
    ...
    try {
        ...
        [
          {
            role: "system",
            content:
              "You are a helpful code analyst. Be concise and insightful.",
          },
      ...
      content.innerHTML =
        '<div style="padding:12px; text-align:center; color:var(--text-muted);"><i class="fa-solid fa-exclamation-triangle" style="color:var(--warning); margin-right:6px;"></i>Could not generate AI summary: ' +
        err.message +
        "</div>";
    }
  },
  async loadLocalFolder(fileList) {
    ...
    document.getElementById("explorer-repo-name").textContent =
      "Local Directory";
    document.getElementById("explorer-lang").textContent =
      Object.keys(filesArray).length + " Files";
    document.getElementById("explorer-stars").innerHTML = "";
    document.getElementById("explorer-forks").innerHTML = "";
