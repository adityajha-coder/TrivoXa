const GithubGitMixin = {
  prepareGitView() {
    ...
    commits.forEach((c) => {
      ...
      contributors[n] = (contributors[n] || 0) + 1;
    });
    ...
    const dayMap = {};
    commits.forEach((c) => {
      const d = new Date(c.commit.author.date).toLocaleDateString("en-US", {
        weekday: "short",
      });
    ...
    document.getElementById("git-insights").innerHTML = `
            <div style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div><span class="text-sm" style="font-weight:600;">Commit Frequency</span><div style="margin-top:10px;display:flex;align-items:flex-end;gap:5px;height:44px;">${[
                  "Mon",
                  "Tue",
                  "Wed",
                  "Thu",
                  ...
                  .map((d) => {
                    ...
                    return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;"><div style="width:100%;height:${pct}%;background:linear-gradient(180deg,var(--primary),var(--primary-dark));border-radius:2px 2px 0 0;min-height:2px;"></div><span class="text-xs text-muted">${d.charAt(0)}</span></div>`;
                  })
                  .join("")}</div></div>
            </div>`;
    ...
    document.getElementById("commits-list").innerHTML = commits
      .slice(0, 18)
      .map(
        (c, i) => `
  ...
  init3DGit() {
    ...
    const branchColors = [
      0x3ecf6e, 0xd4a843, 0xc9952a, 0xe8c547, 0xf0a030, 0xe84545,
    ];
    this.gitNodes = [];
    commits.forEach((c, i) => {
      const x = -i * 2.6;
      ...
      const node = new THREE.Mesh(
        ...
        new THREE.MeshPhongMaterial({
          color,
          emissive: color,
          emissiveIntensity: i === 0 ? 0.35 : 0.12,
        }),
      );
    ...
    branches.slice(1, 5).forEach((b, bi) => {
      ...
      const y = (bi + 1) * 4;
      const si =
        Math.floor(Math.random() * Math.min(6, commits.length - 2)) + 1;
      const sx = -si * 2.6;
      const len = Math.floor(Math.random() * 4) + 2;
      const fp = [
        new THREE.Vector3(sx, 0, 0),
        new THREE.Vector3(sx - 1, y * 0.5, 0),
        new THREE.Vector3(sx - 1.4, y, 0),
      ];
      this.gitScene.add(
        new THREE.Line(
      ...
      for (let j = 0; j < len; j++) {
        ...
        if (j > 0) {
          const lg = new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(sx - 1.4 - (j - 1) * 2.6, y, 0),
    ...
    document.getElementById("git-reset").onclick = () => {
      this.gitCamera.position.set(5, 10, 30);
      this.gitControls.target.set(0, 0, 0);
    };
    this.animateGit();
  },
  animateGit() {
