const ToolsVaultPage = {
  ...
  _applyAuthGate() {
    ...
    if (API.getAuthToken()) {
      ...
      if (extFilter) {
        extFilter.disabled = false;
        extFilter.style.opacity = "";
      }
      const extSearch = document.getElementById("tv-ext-search");
      if (extSearch) {
        extSearch.disabled = false;
        extSearch.style.opacity = "";
      }
      return;
    }
    ...
    const extFilter = document.getElementById("tv-ext-category-filter");
    if (extFilter) {
      extFilter.disabled = true;
      extFilter.style.opacity = "0.4";
    }
    const extSearch = document.getElementById("tv-ext-search");
    if (extSearch) {
      extSearch.disabled = true;
      extSearch.style.opacity = "0.4";
    }
  },
  ...
  bindEvents() {
    document.getElementById("tv-tabs")?.addEventListener("click", (e) => {
      ...
      if (!tab) return;
      document
        .querySelectorAll("#tv-tabs .tab-item")
        .forEach((t) => t.classList.remove("active"));
      tab.classList.add("active");

      const view = tab.dataset.view;
      document.getElementById("tv-resources-view").style.display =
        view === "resources" ? "block" : "none";
      document.getElementById("tv-extensions-view").style.display =
        view === "extensions" ? "block" : "none";
      if (view === "extensions") this.renderExtensionsGrid();
    });

    document
      .getElementById("tv-category-filter")
      ?.addEventListener("change", (e) => {
        this.activeCategory = e.target.value;
        this.renderToolsGrid();
      });

    document.getElementById("tv-search")?.addEventListener(
      "input",
      Helpers.debounce((e) => {
    ...
    document
      .getElementById("tv-ext-category-filter")
      ?.addEventListener("change", (e) => {
        this.activeExtCategory = e.target.value;
        this.renderExtensionsGrid();
      });

    document.getElementById("tv-ext-search")?.addEventListener(
      "input",
      Helpers.debounce((e) => {
  ...
  renderExtensionsGrid() {
    ...
    this.vsCodeExtensions.forEach((group) => {
      if (
        ...
        group.cat === this.activeExtCategory
      ) {
        const filteredItems = group.items.filter((item) => {
